...
#n = 0#1000 #the number of data points the script is behind by

def return_euler(data):
    ...
    latest_data = [euler[index,0], euler[index,1], euler[index,2]]
    return latest_data
...
# Import sensor data
full_data = numpy.genfromtxt("sensor_data.csv", delimiter=",", skip_header=1)
full_data = full_data[0:6000, :]
euler_live_x = []
euler_live_y = []
euler_live_z = []
live_time = []
...
for index in range(1,len(full_data[:,1])):
    
    if time.time() - start_time > 120: #only run for 2 minutes
        break
    
    start_point = index - 50
    if start_point < 0:
        start_point = 0
    
    data = full_data[start_point:index, :]
    euler_angles = return_euler(data)
    euler_live_x.append(euler_angles[0])
    euler_live_y.append(euler_angles[1])
    euler_live_z.append(euler_angles[2])
    live_time.append(full_data[index, 0])
