...
[0,.25,.1,2,5*math.pi/180,5*math.pi/180], [0,.25,.1,3,90*math.pi/180,5*math.pi/180]]#[[0,.25,.1,3,1.25,.01], [0,.25,.1,2,-.517,.05], [0,.25,.1,2,0,.05], [0,.25,.1,3,1.5,.05] ]
    ...
    while end_con == 0:
        ...
        if event_complete == 0: #case 1 of 4 is you're in the middle of a maneuver
            ...
            if next_event_var == 0: #next event based on time
                if time.time() - event_ending_time[current_event] > event_mat[current_event][2]:
